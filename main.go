...
//1. В некоторых местах забыты отладочные принты (например инициализация кэша)
//2. Ошибки не оборачиваются через враппинг, придется искать по подстроке место возникновения ошибки
//3. Контексты не везде прокидываются явно
//4. Используется дефолтный http клиент в конверторе без таймаутов
//5. В запросах к базе данных не используются плейсхолдеры
